...
public class MixinTileEntityEnderChestRenderer extends TileEntitySpecialRenderer<TileEntityEnderChest> implements IMixin
{
    ...
    public void render(final TileEntityEnderChest te, final double x, final double y, final double z, final float partialTicks, final int destroyStage, final float alpha, final CallbackInfo info) {
        if ((x != y || y != z || x != 0.0) && NoRender.EChests.getValue() && ModuleManager.getMod((Class)NoRender.class).isEnabled()) {
            info.cancel();
        }
    }
}
